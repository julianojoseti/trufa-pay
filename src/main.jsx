...
function dueDateValue(sale){ if(!sale?.dueDate) return Number.POSITIVE_INFINITY; const value = new Date(`${sale.dueDate}T00:00:00`).getTime(); return Number.isNaN(value) ? Number.POSITIVE_INFINITY : value; }
function pendingByDueDate(sales){ return [...sales].filter(s=>!s.paid).sort((a,b)=>dueDateValue(a) - dueDateValue(b)); }
function dueReminder(sale){
  if (sale?.paid) return null;
  if (!sale?.dueDate) return { label: 'Sem vencimento', tone: 'none' };
  const todayDate = new Date();
  todayDate.setHours(0,0,0,0);
  const due = new Date(`${sale.dueDate}T00:00:00`);
  if (Number.isNaN(due.getTime())) return { label: 'Sem vencimento', tone: 'none' };
  const diff = Math.round((due.getTime() - todayDate.getTime()) / 86400000);
  if (diff < 0) return { label: `Atrasado ${Math.abs(diff)} dia${Math.abs(diff) === 1 ? '' : 's'}`, tone: 'late' };
  if (diff === 0) return { label: 'Vence hoje', tone: 'today' };
  if (diff === 1) return { label: 'Vence amanha', tone: 'soon' };
  if (diff <= 3) return { label: `Vence em ${diff} dias`, tone: 'soon' };
  return { label: `Vence em ${diff} dias`, tone: 'future' };
}
function customerChargeKey(customer){ return `${normalizeText(customer?.name)}|${normalizePhone(customer?.phone) || normalizeText(customer?.id)}`; }
function resolveCustomerBySale(sale, customers){ return customers.find(customer => customer.id === sale.customerId) || null; }
function customerPendingSales(sales, customers, customer){ const targetKey = customerChargeKey(customer); return sales.filter(sale => !sale.paid && customerChargeKey(resolveCustomerBySale(sale, customers)) === targetKey); }
function whats(salesForCustomer,c,pix,settings,products){ const template = settings?.whatsappTemplate || 'Olá {{customerName}}!\n\nPassando para lembrar do pagamento das trufas.\nValor: {{total}}\nPix: {{pix}}\n\nObrigado!\n{{sellerName}}'; const total = salesForCustomer.reduce((sum,sale)=>sum + Number(sale.total || 0), 0); const productSummary = salesForCustomer.map(sale => saleLabel(sale, products || [])).filter(Boolean).join(' | '); const salesSummary = salesForCustomer.map((sale, index) => `${index + 1}. ${saleLabel(sale, products || [])} • ${money(sale.total)} • ${sale.purchaseDate || sale.date || ''}`).join('\n'); const baseText = formatTemplate(template, { customerName: c?.name || '', total: money(total), pix, sellerName: settings?.sellerName || '', product: productSummary, date: salesForCustomer[0]?.purchaseDate || salesForCustomer[0]?.date || '', saleCount: salesForCustomer.length, salesSummary }); const purchaseNote = salesSummary ? `\n\nTrufas compradas:\n${salesSummary}` : ''; return `https://wa.me/${normalizePhone(c?.phone)}?text=${encodeURIComponent(baseText + purchaseNote)}`; }
function Dashboard({stats,sales,products,customers,settings,markPaid}){ const pending=pendingByDueDate(sales).slice(0,5); return <><section className="hero"><div><span className="tag"><Heart size={18}/>Meta do casamento</span><h2>{money(settings.goal)}</h2><div className="progress"><div style={{width:`${stats.progress}%`}}/></div><p>{money(stats.received)} arrecadado • faltam {money(stats.remaining)} • {stats.progress.toFixed(1)}%</p></div><div className="days"><CalendarDays/><strong>{stats.daysLeft}</strong><span>dias restantes</span></div></section><section className="grid4"><Card icon={DollarSign} title="Recebido" value={money(stats.received)}/><Card icon={Clock} title="Pendente" value={money(stats.pending)}/><Card icon={TrendingUp} title="Lucro estimado" value={money(stats.profit)}/><Card icon={ShoppingCart} title="Total vendido" value={money(stats.total)}/></section><section className="split"><div className="panel"><h2>Pendências</h2>{pending.length?pending.map(s=><SaleRow key={s.id} s={s} sales={sales} customers={customers} products={products} c={customers.find(c=>c.id===s.customerId)} settings={settings} markPaid={markPaid}/>):<p className="empty">Nenhuma pendência.</p>}</div><div className="panel"><h2>Alertas de estoque</h2>{products.filter(p=>p.stock<=p.minStock).map(p=><div className="alert" key={p.id}><AlertCircle/><div><strong>{p.name}</strong><span>Saldo baixo: {p.stock}</span></div></div>)}</div></section></>; }
...
function SaleRow({s,sales,customers,products,c,settings,markPaid,delSale}){ const label = saleLabel(s, products); const pendingSales = customerPendingSales(sales || [], customers || [], c); const reminder = dueReminder(s); return <div className="row"><div><strong>{c?.name}</strong><span>{label} • Compra: {s.purchaseDate || s.date} • Vendedor: {s.sellerName || 'Vendedor'} • Vencimento: {s.dueDate || '-'}</span>{!s.paid && reminder && <small className={`dueHint ${reminder.tone}`}>{reminder.label}</small>}</div><b>{money(s.total)}</b><em className={s.paid?'paid':'pending'}>{s.paid?'Pago':'Pendente'}</em>{!s.paid&&<a className="whats" target="_blank" href={whats(pendingSales,c,settings.pixKey,settings,products)}><MessageCircle size={16}/>Cobrar</a>}{!s.paid&&<button className="mini" onClick={()=>markPaid(s.id)}><CheckCircle2 size={16}/>Pago</button>}{delSale&&<button className="iconBtn" onClick={()=>delSale(s.id)}><Trash2 size={16}/></button>}</div>; }
