...
function SaleRow({s,sales,customers,products,c,settings,markPaid,delSale}){ const label = saleLabel(s, products); const pendingSales = customerPendingSales(sales || [], customers || [], c); return <div className="row"><div><strong>{c?.name}</strong><span>{label} • Compra: {s.purchaseDate || s.date} • Vendedor: {s.sellerName || 'Vendedor'}</span></div><b>{money(s.total)}</b><em className={s.paid?'paid':'pending'}>{s.paid?'Pago':'Pendente'}</em>{!s.paid&&<a className="whats" target="_blank" href={whats(pendingSales,c,settings.pixKey,settings,products)}><MessageCircle size={16}/>Cobrar</a>}{!s.paid&&<button className="mini" onClick={()=>markPaid(s.id)}><CheckCircle2 size={16}/>Pago</button>}{delSale&&<button className="iconBtn" onClick={()=>delSale(s.id)}><Trash2 size={16}/></button>}</div>; }
